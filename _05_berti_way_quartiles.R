...
for (variable in variable_list) {
  # Create binary variables for each tercile
  for (i in 1:4) {
    lower_bound <- as.numeric(quantile(data[[paste0("data.", variable)]], probs = (i - 1) / 4))
    upper_bound <- as.numeric(quantile(data[[paste0("data.", variable)]], probs = i / 4))
...
clustered_data$pamx2_allfactors <- factor(pamx2_allfactors$clustering) # with gower distance
clustered_data$pamx3_allfactors <- factor(pamx3_allfactors$clustering) #with gower distance
...
medoid_allfactors2 <- clustered_data %>%
  mutate(pamx2_allfactors = as.factor(pamx2_allfactors)) %>%
...
medoid_allfactors3 <- clustered_data %>%
  mutate(pamx3_allfactors = as.factor(pamx3_allfactors)) %>%
...
medoid_allfactors4 <- clustered_data %>%
  mutate(pamx4_allfactors = as.factor(pamx4_allfactors)) %>%
