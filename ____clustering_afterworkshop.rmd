---
title: "Clustering - Final version for conference"
author: "Antal Ertl"
date: "`r Sys.Date()`"
...
pca_data_demeaned<- data %>%  mutate(STournamentChoosed =replace_na(STournamentChoosed,0)) %>%
 ...
 # ungroup() %>% 
  ...
  mutate(delta_diff = (delta - mean_delta ) / sd_delta,
        # delta_now_diff = (delta_now - mean_delta_now ) / sd_delta_now,
  ...
  select(studid,classid,delta_diff, 
         beta_diff, #beta_0_diff,
         dictator_diff,dictator_schoolmate_diff,
...
pca_varimax5<- pca_data_demeaned %>%  select(-studid,-classid) %>% psych::principal( nfactors = 5, rotate= "varimax")


pca_varimax5$loadings

data.frame(head(pca_varimax5$loadings, n=nrow(pca_varimax$loadings))) %>%
  mutate(variable = rownames(.)) %>%
  gather(component,loading,-variable) %>%
  # mutate(component = case_when(component == "RC1" ~ "Social Pref. without interactions",
  #                              component == "RC2" ~ "Time Preference",
  #                              component == "RC3" ~ "Risk, risky choice",
  #                              component == "RC4" ~ "Social Pref. with interactions"
  #                              )) %>% 
  ggplot(aes(loading,variable))+
  geom_col(fill = "midnightblue")+
  facet_wrap(~component,nrow=1)+
...
<!-- - RC4.: interakciót igénylő társas preferenciák -->
<!-- - RC2: időpreferencia -->
<!-- - RC1: Interakciót nem igénylő társas preferenciák -->
...
pca_data_demeaned$pca_socpref_interact <- pca_varimax$scores[,"RC1"]
pca_data_demeaned$pca_timepref <- pca_varimax$scores[,"RC2"]
pca_data_demeaned$pca_socpref_nointeract <- pca_varimax$scores[,"RC3"]
pca_data_demeaned$pca_riskpref <- pca_varimax$scores[,"RC4"]
...
rf_rpart <- rpart::rpart(math~delta_diff+
                           beta_diff+
                           dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
...
```{r}
summary(lm(math~delta_diff+dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
...
rf_rpart <- rpart::rpart(read~delta_diff+
                           beta_diff+
                           dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
                           trust_diff+trust_return_diff+
                           comp, data=data_analysis_clean)
...
# Clustering - demeanelt adatokra
 ...
 (Mindegyik esetben 6 klaszternél húztam meg a határt. )
...
```{r}
raw_clustering <- data_analysis_clean %>% select(delta_diff,beta_diff,#beta_0_diff,
                                                 dictator_diff,dictator_schoolmate_diff,risk_diff,publicgood_diff,
                                                 trust_diff,trust_return_diff)
```
...
```{r}
fviz_nbclust(raw_clustering, FUN = hcut, method = "wss")
```
...
```{r}
fviz_nbclust(raw_clustering, FUN = hcut, method = "silhouette")
```
...
gap_stat <- clusGap(raw_clustering, FUN = hcut, nstart = 50, K.max = 15, 
...
plot(dendextend::color_branches(dendagram, k = 5), main="Hierarchical clusters ['Complete' method]", sub ="5 clusters with cut-off value 7.9", leaflab = "none", horiz = F)
...
cut_complete <- cutree(hc.complete, h=7.6)
...
table(clustered_data$hierarchical_cluster)
...
plot(dendextend::color_branches(dendagram, k = 5), main="Hierarchical clusters ['McQuitty' method]", sub ="5 clusters with cut-off value 5.4 (two outlier groups)", leaflab = "none", horiz = F)
abline(h = 5.1, col = 'red')

cut_complete <- cutree(hc.complete, h=5.1)
...
table(clustered_data$hierarchical_mcquitty_cluster)
...
```{r}
fviz_nbclust(raw_clustering, FUN = kmeans, method = "wss")
```
...
```{r}
fviz_nbclust(raw_clustering, FUN = kmeans, method = "silhouette")
```
...
gap_stat <- clusGap(raw_clustering, FUN = kmeans, nstart = 50, K.max = 15, 
                    B = 100) #number of monte-carlo bootstraps
fviz_gap_stat(gap_stat)
...
fviz_cluster(k5, geom = "point",  data = raw_clustering) + ggtitle("K-means clusters with 5 centers and 10,000 simulations")+
  theme_minimal()
...
summary(fpc::pamk(raw_clustering, krange = 3:20,
         metric = "euclidean",
         stand = TRUE,
         critout = TRUE)) # data is standadized; this needs to be true

kmed<- cluster::pam(raw_clustering, k= 5,
         metric = "euclidean",
         stand = TRUE ) # data is standadized; this needs to be true

fviz_cluster(kmed, geom = "point",  data = raw_clustering) + ggtitle("K-medoid clusters with 5 centers and 10,000 simulations")+
  theme_minimal()

clustered_data$kmed_cluster <- kmed$clustering
...
# do for three clusters:

```{r}
set.seed(123469)
k3 <- kmeans(raw_clustering, centers = 3, nstart = 10000)


fviz_cluster(k3, geom = "point",  data = raw_clustering) + ggtitle("K-means clusters with 3 centers and 10,000 simulations")+
  theme_minimal()
...
#k-means:
clustered_data$kmeans_cluster3 <- k3$cluster
...
kmed3<- cluster::pam(raw_clustering, k= 3,
         metric = "euclidean",
         stand = TRUE ) # data is standadized; this needs to be true

fviz_cluster(kmed3, geom = "point",  data = raw_clustering) + ggtitle("K-medoid clusters with 3 centers and 10,000 simulations")+
  theme_minimal()

clustered_data$kmed_cluster3 <- kmed3$clustering


table(kmed3$clustering)

```


Kmedoid vs K-means for three clusters:

```{r}
table(clustered_data$kmed_cluster3,clustered_data$kmeans_cluster3) %>%
  kbl(row.names = TRUE,
      caption  = "k-medoid (row) vs. k-means cluster (columns) with three clusters")%>%
  kable_minimal()
```
