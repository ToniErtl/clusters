...
```{r}
# Define Gower distance as "dist" object:

dist <- daisy(mydata, metric = "gower")
```
...
k-means with gower-ditance

```{r}
fviz_nbclust(mydata, FUN = kmeans, diss = dist, method = "silhouette")
```
...
In order to use it for mixed data with Hierarchical Clustering, we need to use Gower-distance (categorical variables are treated different)
...
gap_stat_hcut <- clusGap(mydata, FUN = hcut, diss = dist, nstart = 50, K.max = 15, 
                    B = 100) 
