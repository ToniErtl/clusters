...
pca_data_demeaned<- data %>%  mutate(STournamentChoosed =replace_na(STournamentChoosed,0)) %>%
  ...
  mutate(delta_diff = (delta - mean_delta ) / sd_delta,
         ...
         #beta_0_diff = (beta_0 - mean_beta_0 ) / sd_ 
  ...
  select(studid,classid,delta_diff,beta_diff, dictator_diff,dictator_schoolmate_diff,
...
```{r}
raw_clustering <- data_analysis_clean %>% select(delta_diff,beta_diff,                                                  dictator_diff,dictator_schoolmate_diff,risk_diff,publicgood_diff,
                                                 trust_diff,trust_return_diff)
```
