...
pca_data_demeaned<- data %>%  mutate(STournamentChoosed =replace_na(STournamentChoosed,0)) %>%
  ...
  # mutate(delta = (delta - mean_delta ) / sd_delta,
  #        beta = (beta - mean_beta ) / sd_beta,
  #        beta_0 = (beta_0 - mean_beta_0 ) / sd_beta_0,
  #        dictator = (dictator - mean_dictator ) / sd_dictator,
  #        dictator_schoolmate = (dictator_schoolmate - mean_dictator_schoolmate ) / sd_dictator_schoolmate,
  #        risk = (risk - mean_risk ) / sd_risk,
  #        publicgood = (publicgood - mean_publicgood ) / sd_publicgood,
  #        trust = (trust - mean_trust ) / sd_trust,
  #        trust_return = (trust_return - mean_trust_return ) / sd_trust_return,
  #        ) %>%
  # select(studid,classid,delta,beta,beta_0,dictator,dictator_schoolmate,
  #        risk,publicgood,trust,trust_return)
  # mutate(delta = (delta / sd_delta) - mean_delta,
  #        beta = (beta / sd_beta) - mean_beta ,
  #        beta_0 = (beta_0 / sd_beta_0) - mean_beta_0 ,
  #        dictator = (dictator / sd_dictator) - mean_dictator ,
  #        dictator_schoolmate = (dictator_schoolmate / sd_dictator_schoolmate) - mean_dictator_schoolmate ,
  #        risk = (risk / sd_risk) - mean_risk,
  #        publicgood = (publicgood / sd_publicgood) - mean_publicgood ,
  #        trust = (trust / sd_trust) - mean_trust ,
  #        trust_return = (trust_return / sd_trust_return) - mean_trust_return ,
  #        ) %>%
  # select(studid,classid,delta,beta,beta_0,dictator,dictator_schoolmate,
  #        risk,publicgood,trust,trust_return)
...
<!-- - Node 7 : <0.5 publicgood és 0.28<=dictator<0.66 (csak ez a két változó érdekes ennél) -->
<!-- - Node 24 : >=0.526 public_good <1.12 ; trust <0.5 és delta >=0.66 -->
<!-- - Node 27 : >=0.526 public_good <1.12 ; trust >=0.5 (itt válik el az előzőtől) és trust <1.123 -->
...
```{r}
summary(lm(math~delta+beta+dictator+dictator_schoolmate+risk+publicgood+
                           trust+trust_return+age+read, data= data_analysis_clean))
```
...
<!-- - Node 7  (N=7; nagyon kicsi klaszter) : <-2.235 publicgood (EXTRÉM alacsony), beta <0.43 és delta < 1.324 (egész magas értékűek) -->
<!-- - Node 21 (N = 97 - ez lényegesen nagyobb klaszter!): 0.43=< publicgood <1.65 és 0.68=<trust<1.27 : EZEK LEHETNEK ESETLEG "RECIPROCITÁS"-CSOPORT? -->

<!-- Sima változók esetében: (összesen kb megint 100 megfigyelés): -->

<!-- - Node 8 (N= 45 ): publicgood és risk (risk tulajdonképpen a beta és delta helyett van az előző fa node 7-éhez képest) -->
...
summary(lm(read~delta+beta+dictator+dictator_schoolmate+risk+publicgood+
                           trust+trust_return+age, data= data_analysis_clean))
```
...
```{r}
raw_clustering <- data_analysis_clean %>% select(delta,beta,beta_0,                                                 dictator,dictator_schoolmate,risk,publicgood,
                                                 trust,trust_return)
```
...
plot(dendextend::color_branches(dendagram, k = 5), main="Hierarchical clusters ['Complete' method]", sub ="5 clusters with cut-off value 150", leaflab = "none", horiz = F)
abline(h = 150.0, col = 'red')


# save the clusters:

# hierarchical clustering:
cut_complete <- cutree(hc.complete, h=150.0)
...
cut_complete <- cutree(hc.complete, h=99)
...
table_one_tech <-data_analysis_clean %>% 
  mutate(kmeans_cluster = as.character(kmeans_cluster)) %>%
  select(kmeans_cluster,delta,beta,dictator,dictator_schoolmate,risk,publicgood,
                           trust,trust_return,comp,
                          pca_socpref_interact,pca_timepref,pca_socpref_nointeract,pca_riskpref,
                          math,read)
table_one <-  arsenal::tableby(kmeans_cluster ~ ., data = table_one_tech) 
...
table(data_analysis_clean$kmeans_cluster, data_analysis_clean$schoolid)
...
table(data_analysis_clean$kmeans_cluster, data_analysis_clean$classid)
...
```{r}
summary(lm(kmeans_cluster~as.factor(schoolid), data = data_analysis_clean))
```


```{r, include = FALSE}
summary(lm(kmeans_cluster~as.factor(schoolid)+as.factor(classid), data = data_analysis_clean))
```
