...
pca_data_demeaned<- data %>%  mutate(STournamentChoosed =replace_na(STournamentChoosed,0)) %>%
  ...
  select(studid,classid,delta_diff, delta_now_diff,
...
pca_data_demeaned$pca_socpref_interact <- pca_varimax$scores[,"RC4"]
pca_data_demeaned$pca_timepref <- pca_varimax$scores[,"RC2"]
pca_data_demeaned$pca_socpref_nointeract <- pca_varimax$scores[,"RC1"]
pca_data_demeaned$pca_riskpref <- pca_varimax$scores[,"RC3"]
...
rf_rpart <- rpart::rpart(math~delta_diff+delta_now_diff+
                           beta_diff+
...
rf_rpart <- rpart::rpart(math~delta+beta+
                           delta_now+dictator+dictator_schoolmate+risk+publicgood+
                           trust+trust_return+
...
```{r}
summary(lm(math~delta_diff+delta_now_diff+dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
...
rf_rpart <- rpart::rpart(math_good~delta+delta_now+
                           beta+
                         +dictator+dictator_schoolmate+risk+publicgood+
                           trust+trust_return+
                           comp, data=data_nomiss)
...
rf_rpart <- rpart::rpart(read~delta_diff+delta_now_diff+
                           beta_diff+
                           dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
                           trust_diff+trust_return_diff+
                           comp, data=data_analysis_clean)
...
data_nomiss <- data %>% 
          dplyr::select(read,delta, delta_now,
                        beta,
                        dictator,dictator_schoolmate,risk,publicgood,
                           trust,trust_return, comp) %>% 
  drop_na()

rf_rpart <- rpart::rpart(read~delta+delta_now+
                           beta+
                           dictator+dictator_schoolmate+risk+publicgood+
                           trust+trust_return+
                           comp, data=data_nomiss)
...
```{r}
summary(lm(read~delta_diff+delta_now_diff+
             beta_diff+
             +dictator_diff+dictator_schoolmate_diff+risk_diff+publicgood_diff+
...
```{r}
summary(lm(read~delta+delta_now+
             beta+
             +dictator+dictator_schoolmate+risk+publicgood+
...
cut_complete <- cutree(hc.complete, h=8)
...
cut_complete <- cutree(hc.complete, h=5.5)
...
table_one_tech <-data_analysis_clean_describe %>% 
  mutate(kmeans_cluster = as.character(kmeans_cluster)) %>%
  select(kmeans_cluster,delta,delta_now,
         beta,
         dictator,dictator_schoolmate,risk,publicgood,
                           trust,trust_return,comp,
                          pca_socpref_interact,pca_timepref,pca_socpref_nointeract,pca_riskpref,
...
table_two_tech <-data_analysis_clean_describe %>% 
  ...
  select(hierarchical_cluster,delta,delta_now,
         beta,
         dictator,dictator_schoolmate,risk,publicgood,
                           trust,trust_return,comp,
                          pca_socpref_interact,pca_timepref,pca_socpref_nointeract,pca_riskpref,
